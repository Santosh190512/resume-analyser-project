    ...
    <section className="mx-auto grid min-h-[calc(100svh-88px)] max-w-7xl items-center px-5 py-10 sm:px-6 lg:px-8">
      <form onSubmit={handleSubmit} className="mx-auto w-full max-w-md rounded-lg border border-white/10 bg-white/[0.08] p-7 shadow-2xl shadow-black/30 backdrop-blur-xl">
        <div className="mb-7 text-center">
          <span className="mx-auto grid h-14 w-14 place-items-center rounded-lg bg-teal-400/15 text-teal-200">
            <ShieldCheck size={28} />
          </span>
          ...
          <p className="mt-2 text-slate-400">
            Continue your resume and job search workflow.
          </p>
        </div>
        ...
        <label className="mb-4 block">
          ...
          <span className="relative block">
            ...
            <input
              ...
              className="h-12 w-full rounded-lg border border-white/10 bg-zinc-950/65 px-11 text-white outline-none placeholder:text-slate-500 focus:border-teal-300/70 focus:ring-4 focus:ring-teal-300/10"
              required
            />
          </span>
        </label>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full rounded-lg bg-teal-400 px-5 py-3 font-bold text-slate-950 shadow-xl shadow-teal-500/20 transition hover:bg-teal-300 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {loading ? "Logging in..." : "Login"}
        </button>

        <p className="mt-5 text-center text-sm text-slate-400">
          New here? <Link to="/register" className="font-semibold text-teal-200">Create account</Link>
        </p>
      </form>
    </section>
  );
